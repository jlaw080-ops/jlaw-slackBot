...
| 수시 | `/작업일지 노트 제목` + 줄바꿈 본문 → 프로젝트 `01_진행업무` 폴더에 노트 | Slack → Obsidian |
...
Slack 입력창에서 **Shift+Enter** 로 줄을 바꿔 이렇게 씁니다.
...
- **첫 줄 = 노트 제목**, 아랫줄 = 본문입니다. (`노트` 없이 그냥 여러 줄만 써도 노트로 저장됩니다)
- 봇이 제목·본문의 낱말을 보고 **프로젝트와 서브 프로젝트를 스스로 골라**
  `01_Projects/02_에너빌드/03_에너지분석/01_진행업무/0904_계산서 검토/0904_계산서 검토.md` 에 저장합니다.
- 못 고르겠으면 **만들지 않고 되묻습니다.** 그때는 첫 줄을
  `계산서 검토 | 에너빌드 | 에너지분석` 처럼 `|` 로 알려 주세요.
- 같은 날 같은 제목으로 또 쓰면 **덮어쓰지 않고** 그 노트의 `## 진행`에 한 줄 덧붙입니다.
- 볼트에 `01_진행업무` 폴더가 없는 프로젝트라면 폴더를 임의로 만들지 않고 알려 줍니다.
  (Obsidian에서 폴더를 먼저 만들어 주세요)

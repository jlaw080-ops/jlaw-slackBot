...
| 퇴근 전 | `/작업일지 일일보고` → 오늘 것을 모아 팀 공유 양식 초안 | Slack + 일일노트 |
| 수시 | `/작업일지 현황` → 업무별 진행 현황 (`배포주소/api/board?secret=…`) | 브라우저·폰 |
...
```
배포주소/api/board?secret=CRON_SECRET값
```

프로젝트별로 무엇이 어디까지 왔는지, **며칠째 멈춰 있는지**를 한 화면에 보여 줍니다.
볼트를 열 때마다 새로 읽으므로 Obsidian Git 동기화를 기다릴 필요가 없습니다.

- Slack에서 `/작업일지 현황` 을 치면 요약 카드와 함께 이 주소가 링크로 나옵니다
- 업무 내용이 담기므로 `CRON_SECRET` 으로 잠겨 있고, 검색엔진에도 노출되지 않습니다
  (`?secret=` 이 붙은 주소를 아무 데나 공유하지 마세요)
- 폰 브라우저 즐겨찾기에 넣어 두면 편합니다

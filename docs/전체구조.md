...
`/작업일지 일일보고` 는 또 다른 블록 `<!-- WORKHUB-REPORT -->` 에 초안을 넣습니다.
...
| `/작업일지 메일` | **볼트** 프로젝트 진행업무 노트 (Gmail 읽기만) | 채널에 결과 |
| `/작업일지 일일보고` | **볼트** 일일노트의 보고 초안 블록 | 채널에 초안 |
| `/작업일지 생성` | **볼트** 일일노트 블록 갱신 | #작업일지에 게시 |
| `/일정 추가` | **Google Calendar** | 채널에 결과 |
| `/티켓 발급` | **볼트** 노트에 `notion: pending` | 채널에 결과 |
| `/작업일지 현황` | 저장 없음 (읽기만) | 요약 + 웹 화면 링크 |
...
| `api/board.ts` | 업무 진행 현황 화면 (`?secret=CRON_SECRET`) |
| `src/lib/vault.ts` | **창고 담당** — 노트 읽기·쓰기, frontmatter, 프로젝트 판정 |
| `src/lib/notes.ts` | 프로젝트 폴더 판정, 진행업무 노트 |
| `src/lib/report.ts` | 일일보고 초안 |
| `src/lib/board.ts` | 업무 진행 현황 (수집 + 화면) |

...
- `core.test.ts` — 날짜 해석, 명령 파싱, 노트 렌더·파싱 왕복 같은 순수 함수
- `vault-io.test.ts` — GitHub 계층을 메모리 파일 시스템으로 바꿔 실제 읽기·쓰기 흐름을 검증합니다.
  토큰 없이도 배포 전에 "봇이 볼트 파일을 어떻게 바꾸는가"를 확인할 수 있습니다.
  본문·frontmatter 키 순서·모르는 키 보존, 일일노트의 봇 블록 밖 내용 보존, Slack 명령이
  실제로 노트를 만들고 바꾸는지까지 봅니다.
